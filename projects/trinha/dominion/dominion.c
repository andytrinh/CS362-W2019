...
int drawAdventurer(int drawntreasure, struct gameState *state, int currentPlayer, int temphand[MAX_HAND], int cardDrawn, int z) {
...
int drawSmithy(int currentPlayer, struct gameState *state, int handPos) {
  //+3 Cards
  int i;
  //for (i = 1; i < 3; i++)
  for (i = 0; i < 3; i++)
...
int drawVillage(int currentPlayer, struct gameState *state, int handPos) {
  //+1 Card
  drawCard(currentPlayer, state);
...
int drawGreatHall(int currentPlayer, struct gameState *state, int handPos) {
  //+1 Card
  drawCard(currentPlayer, state);

  //+1 Actions
  state->numActions++;
  //added a duplicate statement
  state->numActions++;
  //discard card from hand
  discardCard(handPos, currentPlayer, state, 0);
  return 0;
...
int drawTreasureMap(int currentPlayer, struct gameState *state, int handPos, int index) {
...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
  ...
  switch( card )
    ...
    case great_hall:
      drawGreatHall(currentPlayer, state, handPos);
    ...
    case treasure_map:
      drawTreasureMap(currentPlayer, state, handPos, index);
    }
